...
class _BookingScreenState extends State<BookingScreen> {
  ...
  void initState() {
    super.initState();
    initializeDateFormatting('es_ES'); // ← también falta el initState
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('\$${widget.caregiver.price} por cita'),
              ...
              TableCalendar(
                ...
                onDaySelected: (selectedDayValue, focusedDayValue) {
                  final dayName = DateFormat(
                    'EEEE',
                    'es_ES',
                  ).format(selectedDayValue).toLowerCase();

                  final hasAvailability = widget.caregiver.availability.any((
                    availability,
                  ) {
                    return availability.toLowerCase().startsWith(dayName);
                  });

                  if (!hasAvailability) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text("No hay horarios disponibles este día"),
                      ),
                    );
                ...
                Wrap(
                  ...
                  children: widget.caregiver.availability
                      .where((availability) {
                        ...
                        return availability.toLowerCase().startsWith(
                          selectedDayName.toLowerCase(),
                        );
                      })
                      .map((availability) {
                        final slot = availability.split('-')[1];
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () {
                    if (selectedDate == null || selectedSlot == null) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text("Selecciona día y horario"),
                        ),
                      );
